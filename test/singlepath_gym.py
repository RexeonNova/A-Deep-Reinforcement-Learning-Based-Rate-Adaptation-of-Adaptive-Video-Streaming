# version 91 changed from v9, calculate sleeptime using formular and add  SLEEPF event after calculated sleeptime
import math
import numpy as np
...
class SinglepathEnvGym(gym.Env):
    ...
    def reset(self):
        ...
        if self.train:
            ...
            #self.RTT = np.random.uniform(0.05, 0.1)
        else:
...
if __name__ == "__main__":
    np.random.seed(1)
    test = np.asarray(pickle.load(open("bw/fcc_test100kb.pickle", "rb")))
    env = SinglepathEnvGym(bitrate_list=test, train=False)
    env.reset()
    done = False
    reward_mean = 0
    while not done:
