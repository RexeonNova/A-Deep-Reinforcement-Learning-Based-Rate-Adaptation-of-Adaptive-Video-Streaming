...
class Evaluator(object):
    ...
    def eval_saved_agent(self, env, model_path):
        ...
        minError = np.abs(np.mean(eval_scores) - np.min(eval_scores))
        #print(np.min(eval_scores))
        maxError = np.abs(np.max(eval_scores) - np.mean(eval_scores))
        #print(np.max(eval_scores))
        #print(np.mean(eval_scores))
        #print(maxError)
        #print('average evaluation score for model at ' + model_path + ' = ' +str(np.mean(eval_scores)) + " +- " + str(max(minError, maxError)))
        print('average evaluation score for model at ' + model_path + ' = ' +str(np.mean(eval_scores)))
        env.close()
        return np.mean(eval_scores)
    
    def eval_DQN_SB3(self, env, model_path):
        model = DQN.load(path=model_path, env=env, device=self.device)
        avg_rew, _ = evaluate_policy(model=model, env=env, n_eval_episodes=200, return_episode_rewards=True)
        print('average evaluation score for model at ' + model_path + ' = ' +str(np.mean(avg_rew)))
        return np.mean(avg_rew)
    def eval_A2C_SB3(self, env, model_path):
        model = A2C.load(path=model_path, env=env, device=self.device)
        avg_rew, _ = evaluate_policy(model=model, env=env, n_eval_episodes=200, return_episode_rewards=True)
        print('average evaluation score for model at ' + model_path + ' = ' +str(np.mean(avg_rew)))
        return np.mean(avg_rew)
    def eval_PPO_SB3(self, env, model_path):
        model = PPO.load(path=model_path, env=env, device=self.device)
        avg_rew, _ = evaluate_policy(model=model, env=env, n_eval_episodes=200, return_episode_rewards=True)
        print('average evaluation score for model at ' + model_path + ' = ' +str(np.mean(avg_rew)))
        return np.mean(avg_rew)
